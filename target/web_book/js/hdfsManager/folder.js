$(function(){
	...
	$('#dg_hdfsManager_list').datagrid({
		...
		rownumbers : true,// 行号
		pagePosition : 'top',
		url : 'hdfsManager/listFolder.action',
//		url:'',
		queryParams: {
			folder: folder_
		},
	...
	$('#dg_hdfsManager_search').datagrid({
		...
		rownumbers : true,// 行号
		pagePosition : 'top',
		url : 'hdfsManager/searchFolder.action',
		queryParams: {
			folder: "/",
			name:"",
			nameOp:"no",
...
//						return ref_href[0].outerHTML;
	...
	$('#dg_hdfsManager_folder_add_btn').bind('click', function(){
		...
		// ajax 异步提交任务
		var result = callByAJax('hdfsManager/createFolder.action',{folder:folder_});
		if("true" == result.flag){
	...
	$('#dg_hdfsManager_folder_delete_btn').bind('click', function(){
		...
		// ajax 异步提交任务
		var result = callByAJax('hdfsManager/deleteFolder.action',{folder:folder_,recursive:recursive_});
		if("true" == result.flag){
