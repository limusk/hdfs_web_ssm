...
function checkExistAndAuth(folder_,auth_){
	var flag =false;
	var result = callByAJax('hdfsManager/checkExistAndAuth.action',{folder:folder_,auth:auth_});
...
$(function(){
	...
	$('#authUpdateId').bind('click', function(){
		...
		// ajax 异步提交任务
		var result = callByAJax('user/authUpdate.action',{hadoopUserName:hUser,hadoopPassword:hPassword});
		if("true" == result.flag){
