...
//	$('#dg_hdfsManager_file_upload_fileName').hide();
	...
	$('#dg_hdfsManager_file_delete_btn').bind('click', function(){
		...
		// ajax 异步提交任务
		var result = callByAJax('hdfsManager/deleteFile.action',{fileName:file_});
		if("true" == result.flag){
	...
	$('#dg_hdfsManager_file_download_btn').bind('click', function(){
		...
		// ajax 异步提交任务
		var result = callByAJax('hdfsManager/download.action',{fileName:file_,localFile:des_});
		if("true" == result.flag){
	...
	$('#dg_hdfsManager_file_read_btn').bind('click', function(){
		...
		// ajax 异步提交任务
		var result = callByAJax('hdfsManager/read.action',{fileName:file_,textSeq:text_seq,records:records_});
		if("true" == result.flag){
//			$.messager.alert('信息','文件删除成功!','info');
			// 文件读取结果展示；
			$('#retId').show();
